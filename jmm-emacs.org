...
(setf completion-category-overrides
      '((command (styles basic substringMX))
...
(defvar-keymap jmm-x-map
  ...
  "b f" #'browse-url-of-file
  "b b" #'browse-url-of-buffer
  ". d" #'dictionary-search
...
(defun jmm/notmuch-get-email-addresses2 (search-terms)
  "Like `jmm/notmuch-get-email-addresses', but if the address is from Firefox Relay, get the true email.
"
  (let ((args '("address" "--format=sexp" "--format-version=4")))
    (cl-loop for addr1 in (apply #'notmuch-call-notmuch-sexp (append args search-terms))
