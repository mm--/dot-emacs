...
Shamelessly stolen from Sacha Chua
      ...
      (save-excursion
        (save-restriction
          ...
          (when (article-goto-body)
            (let ((process (start-process "espeak-process" " *espeak-gnus*" "espeak" "-a" "200" "-v" "english-us" "-s" "500")))
...
#+begin_src emacs-lisp
  ...
  (defvar josh/helm-org-clock-list
    '("Reddit" "ProbMods" "org-drill" "TV" "Internet" "Programming" "Check Email" "Learn Elisp" "Learn Emacs"))
  (defvar josh/org-clock-in-file "~/org/gtd-test.org")
  
  (defun josh/org-refresh-helm ()
    (interactive)
    (setq josh/helm-org-clock-list 
          (org-map-entries
           (lambda () (nth 4 (org-heading-components)))
           nil
           '("~/org/gtd-test.org"))))
  
  (defvar josh/helm-source-org-clock
    '((name . "Clock in to what Bookmarks")
      (init . josh/org-refresh-helm)
      (candidates . (lambda () josh/helm-org-clock-list))
      (filtered-candidate-transformer
       helm-adaptive-sort)
      (action . (("Clock in"
                  . (lambda (candidate)
                      (josh/org-clock-in candidate)))))))
  
  (defun josh/org-clock-in (taskname)
    "Clock into taskname in gtd-test"
    (interactive)
    (save-excursion
      (let ((place (org-find-exact-headline-in-buffer taskname (find-buffer-visiting josh/org-clock-in-file))))
