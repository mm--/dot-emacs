...
(use-package ess
  ...
  ;; Hmm... this binding isn't working. Maybe ess-mode-map isn't loaded immediately?
...
(use-package notmuch
    :bind (("C-c n" . notmuch-hello)
	   ...
	   ("L f" . jmm/notmuch-search-limit-from-author))
    :config
    (add-to-list 'notmuch-show-insert-text/plain-hook #'jmm/notmuch-wash-proofpoint-links)
    (add-to-list 'notmuch-show-insert-text/plain-hook 'jmm/notmuch-wash-tracking-links))
...
(defun jmm/notmuch-wash-proofpoint-links (msg depth)
  "Decode proofpoint links."
  (goto-char (point-min))
  (while (re-search-forward goto-address-url-regexp nil t)
    (let ((url (match-string 0)))
      (when (string-match-p "urldefense.proofpoint.com" url)
	(replace-match (jmm/url-decode-proofpoint url) nil t)))))
#+END_SRC
