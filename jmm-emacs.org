...
#+BEGIN_SRC emacs-lisp
  ...
  (defun josh/org-get-max-clock-times ()
    "Get the day of the last time we clocked out of a task."
    (let ((times (mapcar #'org-time-string-to-absolute (josh/org-get-end-clock-times))))
      (when times
        (apply 'max times))))
  ...
  (defun josh/org-format-max-clock-time ()
    ...
    (format "[%s]"
            (let ((josh-last-clock-time (josh/org-get-max-clock-times)))
