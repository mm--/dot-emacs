...
(defun jmm/load-personal-files ()
  (let ((personal (concat user-emacs-directory "personal")))
    (when (f-directory-p personal)
      ...
      (mapc 'jmm/org-maybe-tangle-el (directory-files personal t "^[a-zA-Z0-9].*.*org$"))
...
#+begin_src emacs-lisp
(bind-key "M-<return>" 'complete-tag) ;;bind complete-tag.
...
#+begin_src emacs-lisp
  ...
  (bind-key "C-M-<" 'mc/mark-all-like-this-dwim)
...
(defun jmm/pdf-add-annot (color)
  (let ((regiontext (mapconcat 'identity (pdf-view-active-region-text) "\n")))
    (kill-new regiontext)
...
With optional ARG, quote the string."
  (interactive "P")
  (let* ((pdf-text (if pdf-view-active-region
		       (mapconcat 'identity
				  (prog1
				      (pdf-view-active-region-text)
				    (pdf-view-deactivate-region))
