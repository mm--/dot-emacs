...
;; Easily go to specific location from LaTeX -> pdf-tools
...
#+end_src
** Make sure the pdf-tools buffer is up to date
When using ~TeX-view~, I'd like ~pdf-tools~ to turn on ~auto-revert-mode~ so I can see the latest PDF.

Note: Reverting the buffer might not happen in time, so syncing might point to an incorrect area.

#+BEGIN_SRC emacs-lisp
(defun jmm/tex-view-pdf-tools-auto-revert (&rest args)
  "Turn on `auto-revert-mode' in pdf-tools after `TeX-view'."
  (let ((pdf (concat file "." (TeX-output-extension))))
    (with-current-buffer (or (find-buffer-visiting pdf)
			     (find-file-noselect pdf))
      (auto-revert-mode 1))))

(advice-add 'TeX-pdf-tools-sync-view :before #'jmm/tex-view-pdf-tools-auto-revert)
;; (advice-remove 'TeX-pdf-tools-sync-view #'jmm/tex-view-pdf-tools-auto-revert)
...
#+BEGIN_SRC emacs-lisp
(defun jmm/notmuch-search-limit-this-week ()
  "Limit a search to the emails from this week."
  (interactive)
  (notmuch-search-filter "date:\"this week\""))

(defun jmm/notmuch-search-limit-this-month ()
  "Limit a search to the emails from this month."
  (interactive)
  (notmuch-search-filter "date:\"this month\""))
#+END_SRC
...
If you provide KEY-TO-CHECK, pass it through `kbd' first."
  (interactive (list (car (help-read-key-sequence))))
