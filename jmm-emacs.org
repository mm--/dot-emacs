...
(setq-default ispell-complete-word-dict "/usr/share/dict/american-english-large")
#+end_src
...
#+begin_src emacs-lisp
;; AUCTeX
;; (setq load-path (append load-path "/usr/share/emacs23/site-lisp/"))
...
#+begin_src emacs-lisp
  ...
  (setq rcirc-server-alist
      '(("localhost" :port 1799 :nick "emdash" :user-name "joshm" :full-name "Joshua Moller-Mara")))
  ...
  (setq org-capture-templates
        '(("t" "Todo" entry (file+headline "gtd-test.org" "Tasks")
          ...
          ("c" "Calendar" entry (file+headline "gtd-test.org" "Calendar")
           "* %i %?\n\tAdded: %U")
          ("h" "Shopping" entry (file+headline "gtd-test.org" "Shopping")
           "* %i %?\n\tAdded: %U")
...
If PATH is remote, return the remote diretory portion of the path."
	...
	(let ((loc (elt (tramp-dissect-file-name path) 3))
      ...
      (start-process "xterm" " xterm-processes" "xterm" "-T" path "-e" (format "cd %s; bash" path)))))
...
(defun gsd ()
  (interactive)
  ;; (save-excursion
    (find-file-read-only "/home/j2m/Download/gregg/gsd.txt")
    (isearch-forward)
    (start-process "evince-gsd" " gsd-processes" "evince"
		   (concat "--page-label="
			   (number-to-string (- (string-to-number
						...
						12)))
		   "/home/j2m/Download/gregg/gsd.pdf"));; )
