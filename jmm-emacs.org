...
Show playing cards symbols.  You can find supported fonts by using
~fc-list ':charset=1f0bd'~ (this checks fonts for the Queen of hearts
codepoint).

#+begin_src emacs-lisp
;; (set-fontset-font t 'playing-cards "Noto Sans Symbols2") ;; Hard to read at small sizes.
...
#+begin_src emacs-lisp
(add-to-list 'auto-mode-alist '("\\.\\(?:mkiv\\|mkvi\\|mkii\\|mklx\\|mkxl\\)\\'" . context-mode))
(add-to-list 'auto-mode-alist '("\\.lmt" . lua-mode))
#+end_src
...
(use-package eshell
  ...
  (with-eval-after-load "esh-var"
    ...
    (setf (alist-get "owd" eshell-variable-aliases-list nil nil #'equal)
...
* GNUS
   I’m starting to use Gnus more again in 2025.
...
(add-hook 'message-mode-hook 'ruler-mode)
;; `message-mode' derives from `text-mode', which rebinds the default
;; `completion-at-point' binding.  This sets it back.  Useful for
;; completing email addresses or Gnus Gcc headers.
(with-eval-after-load 'message-mode
  (define-key message-mode-map (kbd "C-M-i") 'completion-at-point))
#+end_src
...
#+END_SRC
  ...
  I previously used [[http://msmtp.sourceforge.net/][msmtp]] to automatically handle sending email from different accounts.
  But after reading [[https://www.bounga.org/tips/2020/05/03/multiple-smtp-accounts-in-gnus-without-external-tools/][Nicolas Cavigneaux’s post on configuring multiple SMTP accounts with Gnus]], I’ve switched back to sending email through Emacs with smtpmail.

#+BEGIN_SRC emacs-lisp
;; Previously with msmtp:
...
(setq send-mail-function 'smtpmail-send-it
      smtpmail-default-smtp-server "smtp.gmail.com" ; Until I switch to my own domain.  Soon!
      mail-specify-envelope-from t
      ;; Technically don’t need to set this, since the default
      ;; is to use `send-mail-function'.
...
;; Some URLs don't use query parameters directly
(defun jmm/url-get-query-parameter-in-path (url parameter)
  "Get a pseudo-query parameter that's in the path instead of the query.
That is, an encoded parameter for a URL without a question mark.
URL is a string of the url.
PARAMETER is a string of the parameter."
  (--> url
       (url-generic-parse-url it)
       (car (url-path-and-query it)) ;; Parameters in path (car) instead of query (cdr)
       (url-parse-query-string it)
       (car (alist-get parameter it nil nil #'equal))))
#+END_SRC

Decode ~mail-tracking.hive.co~ links.

#+begin_src emacs-lisp
(defun jmm/url-decode-hivemailtracking (url)
  (if (string-equal "mail-tracking.hive.co" (url-host (url-generic-parse-url url)))
      (jmm/url-get-query-parameter-in-path url "next")
    url))
#+end_src
    ...
    (let* ((url (match-string 0))
	   (decoded-url (save-match-data
			  (thread-first url
			      ...
			      jmm/url-decode-hivemailtracking
