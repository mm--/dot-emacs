...
(use-package org-ql
  ...
  :bind (("C-c A 1" . jmm/org-ql-basic-agenda-1)
	 ...
	 ("C-c A @" . jmm/org-ql-basic-agenda-2-tomorrow)
	 ("C-c A e" . jmm/org-ql-erlich-agenda)
	 ("C-c A E" . jmm/org-ql-erlich-agenda-tomorrow)
...
(defun jmm/org-ql-erlich-agenda-tomorrow ()
  ...
  (org-ql-search (org-agenda-files)
    '(and (and (not (done))
	       ;; Weird, "on tomorrow" isn't working here, but it
	       ;; works for some other agendas?
	       (scheduled :on tomorrow))
	  (tags "erlich")
	  (not (tags "ARCHIVE")))
    :sort '(date priority todo)
...
Taken from the org-ql examples.org, which was shared by reddit user \"emptymatrix\". "
  ...
  (org-ql-search (org-agenda-files)
    ...
    :title "Projects"
    :sort '(date priority)
    :super-groups '((:name "Held projects" :tag "HOLD" :order 6)
		    (:name "Unscheduled projects" :tag "unsched" :order 3)
		    (:name "Stuck projects" :tag "stuck" :order 4)
