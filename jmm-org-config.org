...
When template doesn't have a cdr, delete it."
  (if (listp (car template))		;We're dealing with a list of templates
      (-map 'jmm/add-to-capture template)
	...
	(unless (and parentkey (--find-index (equal (car it) parentkey) org-capture-templates))
	  (jmm/add-to-capture (list parentkey (format "Dummy entry for %s" parentkey))))
	(-if-let (parent-index (--find-index (equal (car it) parentkey) org-capture-templates))
	    (setq org-capture-templates (-insert-at (1+ parent-index) template org-capture-templates))
	  (error "Parent key %s still doesn't exist" parentkey))))))
#+END_SRC
...
(setq org-tag-persistent-alist '(("task" . ?t) ("drill" . ?d)
				 ...
				 ("CANCELLED" . ?C)
                                 ("work" . ?w) ("home" . ?h)
                                 ("REWARD" . ?R) ("SHOPPING" . ?s)
                                 ("paper" . ?p) ("erlich" . ?e) ("NEURO" . ?n) ("nyu" . ?y)
                                 ("vague" . ?v) ("lookup" . ?l)
                                 ("CODING" . ?c)
...
** "vague since" property
   ...
   function adds a timestamp when something is marked "vague".
#+BEGIN_SRC emacs-lisp
(defun jmm/org-vague-since ()
  "Run during `org-after-tags-change-hook'.
If we have a \"vague\" tag and no \"VAGUE_SINCE\" property, add one.
If we don't have a \"vague\" tag but do have a \"VAGUE_SINCE\" property, delete it."
  (let ((current-headline (or (and (org-at-heading-p)
                                   (point))
                              (save-excursion (org-back-to-heading)))))
    (if (member "vague" (org-get-tags-at current-headline))
	(unless (org-entry-get current-headline "VAGUE_SINCE")
	  (org-entry-put current-headline "VAGUE_SINCE" (jmm/org-current-inactive-timestamp)))
      (org-entry-delete current-headline "VAGUE_SINCE"))))
...
(add-hook 'org-after-tags-change-hook 'jmm/org-vague-since)
#+END_SRC
